...
local options = { -- setting default options
    op_start = 0, op_end = 0, ed_start = 0, ed_end = 0, recap_start = 0, recap_end = 0,
}
...
local function skip()
    ...
    -- Check for ending sequence
    if current_time >= options.recap_start and current_time < options.recap_end then
        mp.set_property_number("time-pos", options.recap_end)
    end
end
